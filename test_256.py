...
def preprocess(imgs):
    ...
    for i in range(imgs.shape[0]):
        ...
        #print(np.min(imgs_p[i]))
        #assert 1==2
    imgs_p = imgs_p[..., np.newaxis] #(1120, 1024, 1024, 1)
...
def train_and_predict():
    ...
    print('-'*30)
    model.load_weights('weights/weights-02--0.69--0.68.h5')
    print('-'*30)
    print('Loading and preprocessing test data...')
    print('-'*30)
    
    image_path = '/home/ubuntu-lee/WangXD/2017BOT_TIA/final/2017final_data/stage2_test_500_patch'
    ...
    for image_name in images:
        black = np.zeros((2048, 2048, 1), dtype=np.float32)
        for i in range(15):
            for j in range(15):   
                imgs = np.ndarray((1, 256, 256), dtype=np.uint8)
                ...
                for image in imgs_mask_test:
                    ...
                    black[i*128:i*128+256,j*128:j*128+256,:] = black[i*128:i*128+256,j*128:j*128+256,:] + image
                   
        image = ((black[:, :, 0]>0.5) * 255.).astype(np.uint8)
            #image[0:8,:] = 0
            #image[1016:,:] = 0
            #image[:,0:8] = 0
            #image[:,1016:] = 0
