...
def train_and_predict():
    ...
    image_path = '/home/ubuntu-lee/WangXD/2017BOT_TIA/final/2017final_data/my_test_patch'
    source_image_path = '/home/ubuntu-lee/WangXD/2017BOT_TIA/final/2017final_data/my_test'
    images = os.listdir(source_image_path)

    for image_name in images:
        black = np.zeros((1024, 1024, 1), dtype=np.float32)
        for i in range(7):
            for j in range(7):   
                imgs = np.ndarray((1, 512, 512), dtype=np.uint8)
        ...
        image = ((black[:, :, 0]>2.0) * 255.).astype(np.uint8)
            #image[0:8,:] = 0
            #image[1016:,:] = 0
            #image[:,0:8] = 0
            #image[:,1016:] = 0
